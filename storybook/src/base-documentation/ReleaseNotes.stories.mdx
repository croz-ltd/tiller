...
## **1.18.0 - latest** <span>_(May 19th 2025)_</span>

**<h4>⚡ Component Improvements</h4>**

- `IntlProvider` now supports `defaultRichTextElements` prop, allowing custom rendering of rich text tags (e.g. `<b>`, `<i>`, `<a>`) inside translated messages ([#304](https://github.com/croz-ltd/tiller/issues/304))
- `PageResizer` now accepts a `displayThreshold` prop to control the minimum number of elements required for the component to be shown ([#305](https://github.com/croz-ltd/tiller/issues/305))

## 1.17.0 <span>_(Feb 3rd 2025)_</span>
...
- Fixed console errors for `testId` prop by renaming it to `data-testid` ([#272](https://github.com/croz-ltd/tiller/issues/272))
- Fixed `SidebarNavigation` and `TopNavigation` inconsistencies by fixing incorrect prop drilling on `SidebarNavigation.Dropdown` and `TopNavigation.Dropdown` subcomponents ([#299](https://github.com/croz-ltd/tiller/issues/299))

**<h4>⚡ Component Improvements</h4>**

- Expanded `className` prop to more components and fixed class merging/overriding with `tailwind-merge` ([#122](https://github.com/croz-ltd/tiller/issues/122))
- Added automatic `data-testid` prop value assigning to all field components based on their `name` prop or `value` prop for `CheckboxGroupField.Item` and `RadioGroupField.Item` ([#272](https://github.com/croz-ltd/tiller/issues/272))
- Improved tooltip appearance by adjusting font and adding a `light` theme option via `color` prop ([#297](https://github.com/croz-ltd/tiller/issues/297))
- Added `onSelect` prop to `TopNavigation.Item` and `SidebarNavigation.Item` for custom additional action execution when clicking on an item ([#299](https://github.com/croz-ltd/tiller/issues/299))
- Minimized stutter when clicking the `DropdownMenu` on screen edge ([#295](https://github.com/croz-ltd/tiller/issues/295))

## 1.16.0 <span>_(Dec 17th 2024)_</span>
