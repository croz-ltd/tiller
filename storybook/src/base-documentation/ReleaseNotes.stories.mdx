...
- **Revamped `TreeSelect`** by introducing typing support and improved functionality with additional props ([#282](https://github.com/croz-ltd/tiller/issues/282))
  ...
  - `itemToString` - converts an item of type `T` into a string representation to display the selected item as text in the input field (also used for filtering the options if the `filter` prop is not provided)
