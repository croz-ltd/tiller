...
import tutorial from "img/tokens_tutorial/tokens_import.png";
...
- [Customizing Individual Components](#customizing-individual-components)
...
By changing individual components you can override specific attributes which you need, while preserving everything else from the default theme.
...
Tiller uses the [Phospor icons](https://phosphoricons.com/) icon set by default, but you can easily change it to any other icon set you want
...
Once you have it installed, you can import the token set by following these steps:
...
- Click on `Choose file` and select the downloaded set
- Right-click on the token set and check the `Treat as source` option

<img className="w-full " src={tutorial} alt="Tiller Tokens Import Tutorial" />
...
`primary` set will change the look of most of the components.
