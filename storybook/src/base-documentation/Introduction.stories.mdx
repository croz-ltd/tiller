...
import cover from "img/tiller_cover_2.png";
...
<div className="flex flex-col space-y-4 w-full justify-center items-center">
  ...
  <span className="text-xl text-center">
    Welcome to Tiller Design System - an open-source UI library which offers a set of visual, functional components and
    patterns that accelerate design and development.
  </span>
</div>
...
<video src={tutorial_interface} controls={false} autoPlay={true} muted={true} loop={true} title="Interface Showcase" />
...
<video src={tutorial_factories} controls={false} autoPlay={true} muted={true} loop={true} title="Factories Showcase" />
...
<video src={tutorial_playroom} controls={false} autoPlay={true} muted={true} loop={true} title="Playroom Showcase" />
